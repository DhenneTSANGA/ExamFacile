...
function Premium() {
  return (
    <div>
      <PageHeader title="Passez à Premium" subtitle="Débloquez toute l'expérience ExamFacile et révisez sans limite." />
        ...
        {plans.map((p, i) => (
          <motion.div key={p.name} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}>
            <Card className={`p-8 h-full relative ${p.highlight ? "gradient-brand text-white shadow-glow border-0" : ""}`}>
              ...
              <button className={`w-full py-3.5 rounded-xl font-semibold transition ${p.highlight ? "bg-white text-primary hover:scale-[1.02]" : "bg-card border border-border hover:bg-muted"}`}>
                {p.cta}
              </button>
            </Card>
          </motion.div>
