...
function Leaderboard() {
  ...
  const list = useMemo(() => {
    if (filter === "National") return leaderboard;
    const series = filter.replace("Série ", "");
    return leaderboard.filter((e) => e.series === series).map((e, i) => ({ ...e, rank: i + 1 }));
  }, [filter]);
  ...
  return (
    <div>
      <PageHeader title="Classement national" subtitle="Mesurez-vous aux lycéens de tout le Gabon.">
        <div className="flex gap-2 flex-wrap">
          {filters.map((f) => (
            <button key={f} onClick={() => setFilter(f)} className={`px-4 py-2 text-sm rounded-xl font-medium transition ${filter === f ? "gradient-brand text-white shadow-soft" : "bg-card border border-border hover:bg-muted"}`}>{f}</button>
          ))}
        </div>
      </PageHeader>
          ...
          {[podium[1], podium[0], podium[2]].map((e, i) => {
            ...
            return (
              <motion.div key={e.rank} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: real * 0.1 }} className="text-center">
                ...
                <div className={`${heights[i]} rounded-t-2xl bg-gradient-to-b ${colors[i]} grid place-items-center text-white font-bold text-3xl shadow-soft`}>
                  {e.rank}
                </div>
              </motion.div>
            );
      ...
      <Card className="overflow-hidden">
        ...
        {list.map((e, i) => (
          <motion.div key={e.rank} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.02 }}
